...
fn read_blobs() -> DynResult<Vec<Blob>> {
    ...
    for (name, filename) in blob_config.files {
        ...
        let mut file_size = 0;
        let mut f = File::open(top_dir.join(filename))?;
        loop {
...
fn build_source<F>(out_file: &mut F, blobs: Vec<Blob>, origin: u32) -> DynResult<()>
where
    F: Write,
{
    out_file.write(
...
fn {}() ->  &'static [u8] {{
    unsafe{{slice::from_raw_parts(0x{:x} as *const u8, {})}}
                ...
                blob.name, blob.start + origin, blob.size
            )
            .as_bytes(),
        )?;
    }
    ...
    O: Write,
{
    ...
    let (after, (before, (name, attr, origin, flash_length))) =
        link_script_parser::find_memory_def(&in_buf, "FLASH").map_err(|e| format!("Failed to parse link script: {}", e))?;
    let mut out_buf = before.to_string();
    out_buf += &format!("{} {}: ORIGIN = 0x{:x}, LENGTH = 0x{:x}", name, if let Some(attr) = attr {format!("({})", attr)} else{"".to_string()}, origin, flash_length - length);
    out_buf += after;
...
pub fn prepare_blob() -> DynResult<()> {
    ...
    build_source(&mut source, blobs, u32::try_from(flash_end)?)?;
    Ok(())
}
